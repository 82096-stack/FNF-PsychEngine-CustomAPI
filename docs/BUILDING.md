...
### Graphics Rendering API Selection

You can select a specific graphics rendering API at build time using compile defines:

**Windows:**
```batch
rem Auto-detect (default, prefers DirectX 12)
lime build windows

rem Force DirectX 12
lime build windows -D GRAPHICS_API_DIRECTX12

rem Force Vulkan
lime build windows -D GRAPHICS_API_VULKAN

rem Force OpenGL
lime build windows -D GRAPHICS_API_OPENGL
```

**macOS:**
```bash
# Auto-detect (default, prefers Metal)
lime build mac

# Force Metal
lime build mac -D GRAPHICS_API_METAL

# Force OpenGL
lime build mac -D GRAPHICS_API_OPENGL
```

**Linux:**
```bash
# Auto-detect (default, prefers Vulkan)
lime build linux

# Force Vulkan
lime build linux -D GRAPHICS_API_VULKAN

# Force OpenGL
lime build linux -D GRAPHICS_API_OPENGL
```

You can also change the API at runtime via **Options > Graphics > Graphics Rendering API** (requires game restart).
